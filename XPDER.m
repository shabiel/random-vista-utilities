XPDER ;SFISC/RSD - Rollup Patches into Build ;2015-07-17  9:42 PM
...
GO ;
 ...
 Q
ROLL ; Generic Roller upper
 N Y
 N Z S Z="AEMQZL"
 Q:'$$DIC^XPDE(.Z,"Rollup patches into Build: ")
 N XPDA,XPDNM,XPDPKG,XPDVER
 S XPDA=+Y,XPDNM=$P(Y(0),U),XPDPKG=+$P(Y(0),U,2),XPDVER=$$VER^XPDUTL(XPDNM)
 ;
 N X R !!,"Roll which patches? ",X:$G(DTIME,300)
 I X=""!X["^" QUIT
 ;
 N XPDT
 N ORIG S ORIG=X
 F  S X=$O(^XPD(9.6,"B",X)) Q:X=""  Q:(ORIG'=$E(X,1,$L(ORIG)))  D
 . N IEN S IEN=$O(^XPD(9.6,"B",X,""))
 . Q:IEN=XPDA
 . W "Rolling "_X,!
 . S XPDT(IEN)=""
 ;
 N DIR S DIR(0)="Y",DIR("A")="Ready" D ^DIR
 I Y DO GO
 QUIT
 ;
